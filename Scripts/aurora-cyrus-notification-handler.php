...
if ($bDebug) {
    ...
    $rawEventData = file_get_contents($DIR . 'event.txt');
} else {
    $rawEventData = file_get_contents('php://stdin');
}

$sMessage = '';
if ($rawEventData) {
    $oEventData = json_decode(str_replace(["\r","\n"], '', $rawEventData));

    $sMessage = $oEventData->messageContent ?? '';
}
...
function getRecipient($sMessage)
{
    $sRecipient = '';
    preg_match("/^(?:\s*to):\s*(.+)$/im", $sMessage, $aMatch);
    if (isset($aMatch[1])) {
        $sRecipient = trim($aMatch[1]);
    }
    return $sRecipient;
};
function getSender($sMessage)
{
    $sRecipient = '';
    preg_match("/^(?:\s*from):\s*(.+)$/im", $sMessage, $aMatch);
    if (isset($aMatch[1])) {
        $sRecipient = trim($aMatch[1]);
    }
    return $sRecipient;
...
{
    $sSubject = '';
    preg_match("/^(?:\s*subject):\s*(.+)$/im", $sMessage, $sSubjectMatch);
...
{
    $sId = '';
    preg_match("/^(?:\s*message-id):\s*(.+)$/im", $sMessage, $sMatch);
    if (isset($sMatch[1])) {
        $sId = trim($sMatch[1]);
    }
    return $sId;
...
$sRecipient = getRecipient($sMessage);
$sSender = getSender($sMessage);
$sMessageSubject = getMessageSubject($sMessage);
$sMessageId = getMessageId($sMessage);

/* === Output debug info ==== */
$logger("=== Process incomming message ===");
$logger("Recipient:", $sRecipient);
$logger("Message id:", trim($sMessageId));
...
if (!$CONFIG_AURORA_URL) {
    ...
    // WHERE a.Email === '$sRecipient' AND u.PublicId = '$sRecipient'";
    ...
    //     $logger("No params found for mail account:", $sRecipient);
    //     exit(0);
    // }
